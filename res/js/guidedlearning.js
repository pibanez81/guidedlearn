...
for (var index = 0; index < lengthofLoop; ++index) {
    ...
    if ((json[index].mode) == 2){
        ...
        htmlData += '<td valign="top"><a href="{{url_home}}/pages/' + json[index].name + '/' + json[index].name + '.html" target="_blank"><img src="{{url_image}}' + json[index].name + '.jpg" class="imgBox" /></a></td>';
    }
}
